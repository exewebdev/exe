        ...
        sql.query("SELECT email FROM Member WHERE email=?", [user.email], function(error, rows, fields) {
            ...
            if (rows[0] !== undefined) {
                ...
                sql.query("INSERT INTO Member SET ?", [user], function(error, rows) {
                    ...
                    sql.query("SELECT member_id FROM Member WHERE email=?", [user.email], function(error, rows) {
                        //Adds password to credidentials table.
                        sql.query("INSERT INTO Credential SET ?", [{member_id: rows[0].member_id, password:password}], function(error, rows) {
                            if (error){
                                console.log(error);
                            }
                            onsuccess();  
